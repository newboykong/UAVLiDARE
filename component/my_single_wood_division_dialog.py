...
from ui.ui_single_wood_division_dialog import Ui_Dialog as SingleWoodDialog
from PySide6.QtWidgets import QTreeWidgetItem
from PySide6.QtCore import Qt, Slot
from functools import partial


class MySingleWoodDialog(QDialog):
    def __init__(self, parent=None):
        super().__init__(parent)
        # 把ui转为py文件在此接入
        self.ui = SingleWoodDialog()
        self.ui.setupUi(self)
        self.params=None
        self.name= None
        self.ui.select_file_tif.clicked.connect(partial(self.file_save_tif))
        self.ui.select_file_shp.clicked.connect(partial(self.file_save_shp))
    ...
    def add_items(self, params_dic):
        ...
        self.ui.save_file_path_tif.clear()
        self.ui.save_file_path_shp.clear()
    ...
    def params_get(self):
        params_dic = {'chm_file_path': str(self.ui.chm_file_path.text()),
                      'save_file_path_tif':str(self.ui.save_file_path_tif.text()),
                      'save_file_path_shp':str(self.ui.save_file_path_shp.text())}
    ...
    def file_save_tif(self):
        # 让用户选择保存路径
        self.name=self.remove_las_tif_suffix(self.name)
        file_path, _ = QFileDialog.getSaveFileName(self, "另存为 SingWoodDivision 文件", self.name + "-单木分割",
                                                   "tif 文件 (*.tif)")
        if not file_path:
            return
        self.ui.save_file_path_tif.setText(file_path)

    def file_save_shp(self):
        # 让用户选择保存路径
        self.name=self.remove_las_tif_suffix(self.name)
        file_path, _ = QFileDialog.getSaveFileName(self, "另存为 SingWoodDivision 文件", self.name + "-单木分割",
                                                   "shp 文件 (*.shp)")
        if not file_path:
            return
        self.ui.save_file_path_shp.setText(file_path)
